...
class Heap:
    ...
    def __largestChild(self, index, lastIndex):
        ...
        if (2 * index + 1) > lastIndex:
            ...
            if self.data[2 * index + 1] > self.data[2 * index + 2]:    # what if their equal??
                return 2 * index + 1
            else:
                return 2 * index + 2
    ...
    def __siftDownFromTo(self, fromIndex, last):
        ...
        ### WRITE YOUR CODE HERE###
        while last - 1 > (2 * fromIndex + 1):
            largestChild = self.__largestChild(fromIndex, last)
            if largestChild:
                self.data[fromIndex], self.data[largestChild] = self.data[largestChild], self.data[fromIndex]
                fromIndex = largestChild
            else:
                break
        return 
...
def main():
    ...
    # values = [10, 30, -100, 50, 20, 30, -40, 70, 5, 50]
