...
class Object:
  ...
  def getReflection(self, ray, intersection_point):
    # Getting T and N value
    T = normalize(ray)
    N = self.getNormal(intersection_point)
  ...
  def getColor(self, intersection_point):
  ...
  def phongIntensity(self, ray, intersection_point):
    # Computes Light vector from point_light to intersect point
    L = computeUnitVector(intersection_point, light_source)
  ...
  def getNormal(self, intersection_point):
    if (type(self) == Plane):   # If plane then call and return plane normal
      ...
      int_point, pos = intersection_point, self.get_position()
...
class Sphere(Object):
  ...
  def intersect(self, ray_origin, ray):
    ...
    if discriminant < 0:    # No Roots
      ...
      return []     # Returns no intersection
      ...
      if Z > horizon or Z < 0 or self.get_t() < 0.001: 
        ...
        return [X,Y,Z]    # Returns intersection point
...
def traceRay(start_point, ray, depth):
  ...
  for object in Object.all_objects:
    test_intersection_point = object.intersect(start_point, ray)
    if test_intersection_point != []:   # Does Ray Intersect Object?
      if object.get_t() < tMin:                         # Determines Closest Object
          tMin = object.get_t()
          nearest_object = object                  # Sets Current Object as Nearest Object
          intersection_point = copy.deepcopy(test_intersection_point)
  if tMin == 999999: return sky_color                # No Intersect = Sky 
  # Get local object color
  color = nearest_object.getColor(intersection_point)
  # Get intesity of current
  intensity = nearest_object.phongIntensity(ray, intersection_point)
  # Determines if point is in shadow
  if inShadow(nearest_object, intersection_point): intensity *= 0.25
  ...
  if nearest_object.get_reflect() != 0: reflection_vector = nearest_object.getReflection(ray, intersection_point)
  reflect_color = traceRay(intersection_point, reflection_vector, depth-1)
